...
namespace CompasAiCad
{
    ...
    public sealed class Library
    {
            ...
            [In, MarshalAs(UnmanagedType.IDispatch)] object kompas_)
        {
            ...
            {
                MessageBox.Show("AI CAD не смог открыть панель.\n\n" + ex.Message,
                    "Compas AI CAD", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private static bool IsNativePanelEnabled()
        {
            string value = Environment.GetEnvironmentVariable("COMPAS_NATIVE_PROPERTY_PANEL");
            return string.Equals(value, "1", StringComparison.OrdinalIgnoreCase)
                || string.Equals(value, "true", StringComparison.OrdinalIgnoreCase)
                || string.Equals(value, "yes", StringComparison.OrdinalIgnoreCase);
        }

        private static bool TryShowNative(object kompas)
        {
            lock (Sync)
            {
                _native = _native ?? new PropertyManagerBackend();
                return _native.TryOpen(kompas);
            }
        }
        ...
        public void ClosePanel()
        {
            lock (Sync)
            {
                ...
                if (_form == null || _form.IsDisposed)
                    return;
                try { _form.BeginInvoke((Action)(() => _form.Hide())); } catch { }
            }
        }
        ...
        private sealed class PanelForm : Form
        {
            ...
            public PanelForm(IntPtr parent)
            {
                ...
                var header = new Panel { Dock = DockStyle.Top, Height = 42, Padding = new Padding(10, 6, 8, 6) };
                var title = new Label { Text = "AI CAD", Dock = DockStyle.Fill, Font = new System.Drawing.Font("Segoe UI", 12F, System.Drawing.FontStyle.Bold), TextAlign = System.Drawing.ContentAlignment.MiddleLeft };
                var close = new Button { Text = "×", Dock = DockStyle.Right, Width = 32, FlatStyle = FlatStyle.Flat, Font = new System.Drawing.Font("Segoe UI", 13F), TabStop = false };
                close.FlatAppearance.BorderSize = 0; close.Click += (s, e) => HidePanel();
                header.Controls.Add(title); header.Controls.Add(close);

                var hint = new Label { Text = "Описание детали или команда изменения", Dock = DockStyle.Top, Height = 30, Padding = new Padding(10, 5, 8, 0) };
                _task = new TextBox { Multiline = true, ScrollBars = ScrollBars.Vertical, Dock = DockStyle.Top, Height = 150, Font = new System.Drawing.Font("Segoe UI", 10F), AcceptsReturn = true };
                var buttons = new FlowLayoutPanel { Dock = DockStyle.Top, Height = 46, Padding = new Padding(8, 5, 8, 5), WrapContents = false };
                ...
                buttons.Controls.Add(_create); buttons.Controls.Add(_edit); buttons.Controls.Add(_save);

                _status = new Label { Text = "Готово", Dock = DockStyle.Top, Height = 28, Padding = new Padding(10, 5, 8, 0) };
                _log = new TextBox { Multiline = true, ReadOnly = true, ScrollBars = ScrollBars.Both, Dock = DockStyle.Fill, Font = new System.Drawing.Font("Consolas", 9F) };
                Controls.Add(_log); Controls.Add(_status); Controls.Add(buttons); Controls.Add(_task); Controls.Add(hint); Controls.Add(header);
            ...
            public void SetParentWindow(IntPtr parent)
            {
                ...
                SetParent(Handle, parent); AttachAndLayout();
            }

            private void AttachAndLayout()
            {
                if (_parent == IntPtr.Zero || !IsHandleCreated) return;
                if (Parent == null) SetParent(Handle, _parent);
                int style = GetWindowLong(Handle, GWL_STYLE);
                int desired = style | WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN;
                if (style != desired) SetWindowLong(Handle, GWL_STYLE, desired);
                if (!GetClientRect(_parent, out RECT r)) return;
                ...
                SetWindowPos(Handle, IntPtr.Zero, r.Right - width, 0, width, height, SWP_NOACTIVATE | SWP_SHOWWINDOW);
            }

            private async Task RunAsync(string action)
            {
                if (_running) return;
                if (action != "save" && string.IsNullOrWhiteSpace(_task.Text)) { Append("Введите описание детали."); return; }
                _running = true; SetButtons(false);
                _status.Text = action == "save" ? "Сохранение…" : "Построение…";
                string repo = FindRepo(); string python = FindPython(repo);
                ...
                try
                {
                    ...
                    Append(result.Output); if (!string.IsNullOrWhiteSpace(result.Error)) Append(result.Error);
                    _status.Text = result.ExitCode == 0 ? "Готово" : "Ошибка";
                }
                catch (Exception ex) { _status.Text = "Ошибка"; Append(ex.ToString()); }
                finally { try { if (File.Exists(taskFile)) File.Delete(taskFile); } catch { } _running = false; SetButtons(true); }
            }

            private static ProcessResult ExecutePython(string repo, string python, string action, string taskFile)
            {
                string args = action == "save" ? "-m addon.bridge save" : $"-m addon.bridge {action} --task-file \"{taskFile}\"";
                var psi = new ProcessStartInfo { FileName = python, WorkingDirectory = repo, UseShellExecute = false, CreateNoWindow = true, RedirectStandardOutput = true, RedirectStandardError = true, Arguments = args };
            ...
            private void SetButtons(bool enabled) { if (IsDisposed) return; _create.Enabled = enabled; _edit.Enabled = enabled; _save.Enabled = enabled; }
            private void HidePanel() { Visible = false; _status.Text = "Готово"; }

            private static string FindPython(string repo)
            {
                string[] candidates = { Path.Combine(repo, "venv", "Scripts", "python.exe"), Environment.GetEnvironmentVariable("COMPAS_PYTHON"), "python.exe" };
                foreach (string item in candidates) if (!string.IsNullOrWhiteSpace(item) && File.Exists(item)) return item;
                return "python.exe";
            }
            ...
            private void Append(string text)
            {
                ...
                try { _log.AppendText(text + Environment.NewLine); _log.SelectionStart = _log.TextLength; _log.ScrollToCaret(); } catch { }
            }
        }
