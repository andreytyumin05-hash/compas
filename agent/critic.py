...
_FORBIDDEN = (
    ...
    "loft(", "sweep(", "shell(", "thread(", "sketch_on_face(",
)
...
def is_complex_task(task: str) -> bool:
    ...
    if any(w in t for w in (
        "ступен", "уступ", "пробк", "штуцер", "шестигран", "канавк", "карман",
        "бобыш", "feature=", "build_plan", "body_style=cylindrical", "required_features=",
        "лопаст", "spline", "bezier", "параметр",
    )):
        return True
    if re.search(r"\bвал\b", t):
        return True
    diams = re.findall(r"(?:ø|∅|diameter\s*=)\s*(\d+(?:[.,]\d+)?)", t)
...
def extract_sketch_calls(code: str) -> List[str]:
    return re.findall(r"\bsk\.([A-Za-z_]\w*)\s*\(", code or "")


def unknown_part_calls(code: str) -> List[str]:
    return list(dict.fromkeys(n for n in extract_part_calls(code) if n not in KNOWN_PART_METHODS))


def unknown_sketch_calls(code: str) -> List[str]:
    return list(dict.fromkeys(n for n in extract_sketch_calls(code) if n not in _SKETCH_METHODS))
...
def review_before_build(task: str, code: str, *, llm: Any = None, use_llm: Optional[bool] = None) -> Tuple[bool, List[str]]:
    issues = review_structure(task, code)
    if issues:
        return False, issues
