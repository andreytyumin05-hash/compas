...
3. Несколько тел: несколько sketch+extrude подряд (база → бобышка → ступень → pocket).
4. Отверстия: hole или cut, не «второй circle в одном extrude с контуром».
5. Для сложной крышки: сначала базовый контур + extrude, потом boss на центре, затем blind pocket cut, затем pattern_holes_circular/rect. Не смешивать базу и вырез в одном эскизе.
6. Если нужно выбрать плоскость, используйте xy/xz/yz явно, а не подразумеваемую проекцию.
'''
